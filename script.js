...
document.querySelector('#set-grid-bgColor').addEventListener('input', (e) => {
...
function setMode(e) {
  ...
  switch (currentMode) {
    ...
    case 'brighteningEffect':
      e.target.style.backgroundColor = getLightnessEffect(e);
      break;
  }
};

function highlightCurrentSquare(e) {
  let rgbValues = getRgbValues(e.target.style.backgroundColor);
...
function getRgbValues(color) {
  ...
  if (color === 'transparent') {
    rgbValues = grid.style.backgroundColor.match(rgbRegExp);
  } else {
    rgbValues = color.match(rgbRegExp);
  }
  return rgbValues;
};
...
function getLightnessEffect(e) {
  let rgbValues = getRgbValues(e.target.style.backgroundColor);
  let subtractedValues;
  switch (currentMode) {
    case 'darkeningEffect':
      subtractedValues = rgbValues.map(value => value - 10);
      break;
    case 'brighteningEffect':
      subtractedValues = rgbValues.map(value => +value + 10);
      break;
...
document.querySelector('#eraseGrid').addEventListener('click', () => {
  ...
  if (answer && answer == 'y' || answer == 'yes') {
    ...
    for (let square of drawingSaved) {
      square = 'transparent';
    }
  } else {
    return;
